...
class bibitem():
    ...
    def __init__(self,lines,new_item=False,debug=0):
        ...
        if  new_item:
            ...
            if '"' in self.Shortcut_NameYear:
                self.Shortcut_NameYear =self.Shortcut_NameYear[1:-1]
            self.Shortcut_Multiple = ''
...
#
    ...
    parser.add_option("-f", "--file", dest="newfile", action = "store", default = "newbib.bib")
    ...
    parser.add_option("-i", "--infile", dest="infile", action = "store", default = "ms.bib")
    ...
    if options.lookup == None:
        if options.url is None:
            newbib, newauthors = scrubfile(options.newfile,new_item=True)
        elif options.lookup is None:
            newbib, newauthors = scrubhtml(options.url,new_item=True)

    if not options.test:
        oldbib, authors = scrubfile(options.infile,new_item=False)
