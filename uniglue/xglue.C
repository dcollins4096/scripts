...
int main(int argc, char **argv)
{
  ...
  char *logname = new char[strlen(FieldName)+4+1];
  strcpy(logname, FieldName);
  strcat(logname, ".log");
  log = fopen(logname, "w");
  ...
  n=gridcounter;
    ...
    strcat(DumpName, pid);
    printf("Get numeric type of input file %s\n", DumpName);

  file_id = H5Fopen(DumpName, H5F_ACC_RDONLY, H5P_DEFAULT);
    assert( file_id != h5_error );
  dset_id = H5Dopen(file_id, FieldName);
  ...
  if ( H5Tequal( file_type_id, H5T_STD_B8BE ) )
  {
    file_type_id = HDF5_FILE_B8;
    mem_type_id = HDF5_R16;
    dtype = 4;
  }
  ...
  int *output_buffer = new int[gbuff_size*dtype];
  ...
  for(gridcounter = 0; gridcounter < ngrids; gridcounter++)
  {
    ...
    int *input_buffer = new int[dbuff_size*dtype];

    h5_status = H5Dread(in_dset_id, mem_type_id, in_mem_dsp_id, in_file_dsp_id, H5P_DEFAULT, (VOIDP) input_buffer);
      assert( h5_status != h5_error );
...
*/
    ...
    for (k = StartIndex[2]; k <= EndIndex[2]; k++)
    {
      for (j = StartIndex[1]; j <= EndIndex[1]; j++)
      {
        ...
        file_stride[2] = 1;
        file_count[2] = EndIndex[0]-StartIndex[0]+1;

        mem_offset = 0;
        mem_stride = 1;
        mem_count = EndIndex[0]-StartIndex[0]+1;
        ...
        h5_status = H5Sselect_hyperslab(file_dsp_id, H5S_SELECT_SET, file_offset, file_stride, file_count, NULL);
          assert( h5_status != h5_error );
        h5_status = H5Dwrite(dset_id, mem_type_id, mem_dsp_id, file_dsp_id, H5P_DEFAULT, (VOIDP) output_buffer);
          assert( h5_status != h5_error );
